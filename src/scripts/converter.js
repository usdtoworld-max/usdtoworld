...
function initConverter() {
  ...
  async function runConversion() {
    ...
    try {
      ...
      if (trendEl) {
        const key = `usdtoworld:prevrate:${from}${to}`;
  ...
  function recordRecent(from, to, amount) {
    try {
      const list = JSON.parse(localStorage.getItem('usdtoworld:recent') || '[]');
      const entry = { from, to, amount, ts: Date.now() };
      const filtered = list.filter((e) => !(e.from === from && e.to === to));
      filtered.unshift(entry);
      localStorage.setItem('usdtoworld:recent', JSON.stringify(filtered.slice(0, 5)));
      renderRecent();
    } catch (e) {}
  }
  ...
  // Allow deep-linking a pair via ?from=USD&to=CNY (used by error pages, quick links, etc.)
  try {
    const params = new URLSearchParams(window.location.search);
    const qFrom = params.get('from');
    const qTo = params.get('to');
    const qAmount = params.get('amount');
    if (qFrom && [...fromSelect.options].some((o) => o.value === qFrom)) fromSelect.value = qFrom;
    if (qTo && [...toSelect.options].some((o) => o.value === qTo)) toSelect.value = qTo;
    if (qAmount && isFinite(parseFloat(qAmount))) amountInput.value = qAmount;
  } catch (e) {}
